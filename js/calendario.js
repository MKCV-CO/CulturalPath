...
const createPalestra = (palestra) => {
  ...
  const dataString = palestra.data_palestra;
  ...
  // Formatar dia e mês sem zero à esquerda, se necessário
  const diaFormatado = dia.startsWith("0") ? dia.substring(1) : dia;
  const mesFormatado = mes.startsWith("0") ? mes.substring(1) : mes;

  let card = {
    day: Number(diaFormatado),
    month: Number(mesFormatado),
    year: Number(ano),
    events: [
      {
        title: `${palestra.tema}`,
...
export const loadPalestra = async () => {
  ...
  const cards = palestra.map(createPalestra)
